...
function ScoreCard({
  ...
  timeTaken
    ...
    const maxPoints = noq * 10;
    const displayTimeTaken = timeTaken ?? 'NA';
  ...
  return (
    <div className="card mx-auto flex flex-col rounded-xl lg:w-[500px]">
      ...
      <span className="mb-4 w-full text-center text-3xl font-bold text-primary drop-shadow-xl dark:text-secondary lg:text-4xl">
         SCORE {obtainedPoints}/{maxPoints}
      </span>
      {/* Progress Bar */}
       <div className="mb-4 w-full bg-gray-300">
       <div
             className="bg-primary h-4 rounded"
             style={{ width: `${(obtainedPoints / maxPoints) * 100}%` }}
             />
       </div>
      <hr className="mb-6 h-px border-0 bg-gray-400 dark:bg-gray-600" />
      {/* Additional Information */}
          <div className="marks">
              <div className="justify-self-start">Time Taken</div>
              <div>{displayTimeTaken} minutes</div>
          </div>
