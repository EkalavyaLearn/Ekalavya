...
import { placeholder, standardQuizImage } from '../../assets';
...
function Thumbnail({ title, submissions, id, noq, type }) {
  ...
  let thumbnailLink;
  if (type === 'video') {
    thumbnailLink = `http://img.youtube.com/vi/${id}/maxresdefault.jpg`;
  } else {
    thumbnailLink = standardQuizImage; // Using a standard quiz image from the project
  }
    ...
    >
      <div className="card relative aspect-video w-full overflow-hidden rounded-lg p-0">
        <img
          ...
          onLoad={() => setImageLoaded(true)} // Corrected the logic here
        />
